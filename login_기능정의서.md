...
```mermaid
flowchart TD
    A[로그인페이지/비밀번호찾기 클릭]-->B{아이디 입력}-->c[회원시 등록된 휴대폰 확인 ex)010-7***-7***]-->d[이름/휴대번호입력]-->e[인증번호 받기]-->f[인증번호 6자리입력]-->g[다음]-->h[가입된 이메일로 암호변경 링크 발송] -->b{없는 아이디 입력}-->h[아이디 확인 요청]-->i[로그인 페이지로 돌아감]
```
